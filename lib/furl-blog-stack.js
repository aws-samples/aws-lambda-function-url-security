...
const { Policy } = require('aws-cdk-lib/aws-iam');
...
class FurlBlogStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        ...
        reason: 'The cognito AdvancedSecurityMode suppression error is a new addition and theres the CDK construct still doesnt support this feature'
      },
    ])
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        ...
        reason: 'There are seveal IAM roles created in this stack that are restrcited by what "actions" they are allowed to perform however the resources themselves are created later. Using * to avoid having complex dependencies in this CDK stack.'
      },
    ])
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        id: 'AwsSolutions-L1',
        reason: 'CustomResources are being used in this stack. CDK is choosing to use Node 12 as opposed to Node 16 - suppressing as theres no way to force CDK to use latest runtime. Read here: https://github.com/aws/aws-cdk/pull/20531'
      },
    ])
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        ...
        reason: 'Were disabling access logging and TLS version enforcement since this is a demo app and we want end users with older browsers to be access the website well host.'
      },
    ])
...
// step 2.0: create a service role (and policy)to attach to amplify app

const amplifyAppRole = new iam.Role(this, 'amplify-app-service-role', {
  assumedBy: new iam.ServicePrincipal('amplify.amazonaws.com'),
});

const amplifyAppPolicy = new iam.PolicyStatement({
  actions: [
...
amplifyAppRole.addToPolicy(amplifyAppPolicy);
...
var cloduWatchPolicy = new iam.PolicyStatement({
  actions: ["logs:CreateLogGroup", "logs:GetLogEvents", "logs:PutLogEvents"],
  effect: iam.Effect.ALLOW,
  resources: '*'
});
...
const cognitoPresignUpExecRole = new iam.Role(this, 'cognito-presignup-function-role', {
  assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
});

//cognitoPresignUpExecRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaBasicExecutionRole"));
cognitoPresignUpExecRole.addToPolicy(cloduWatchPolicy);

userPool.addTrigger(cognito.UserPoolOperation.PRE_SIGN_UP , new lambda.Function(this, 'lambdaFurlPreSignUpTrigger', {
  runtime: lambda.Runtime.NODEJS_16_X,
  handler: 'index.handler',
  role: cognitoPresignUpExecRole,
...
lambdaFnPolicy.addActions('logs:*');
lambdaFnPolicy.addResources('*');

const lambdaWithFurlRole = new iam.Role(this, 'lambdawithfurl-function-role', {
  assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
});

lambdaWithFurlRole.addToPolicy(lambdaFnPolicy);
